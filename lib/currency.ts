...
export interface CurrencyConfig {
  ...
  name: string;
  flag: string;
  proPrice: number; // One-time price
  teamsPrice: number; // Monthly price per user
}
...
/**
 * Auto-detect user currency based on system timezone and browser locale.
 */
...
/**
 * Persist user explicit currency choice.
 */
...
/**
 * Formats amount with proper symbol and locale formatting.
 */
...
/**
 * Returns formatted price string for Stickle Pro Supporter.
 */
export function getProPriceFormatted(currencyCode: CurrencyCode = detectUserCurrency()): string {
  const config = SUPPORTED_CURRENCIES[currencyCode] || SUPPORTED_CURRENCIES.USD;
  return formatPrice(config.proPrice, config.code);
}

/**
 * Returns formatted price string for Stickle Teams per month.
 */
export function getTeamsPriceFormatted(currencyCode: CurrencyCode = detectUserCurrency()): string {
  const config = SUPPORTED_CURRENCIES[currencyCode] || SUPPORTED_CURRENCIES.USD;
  return formatPrice(config.teamsPrice, config.code);
}
